  ...
  return ( 
    <div className='flex flex-col justify-center items-center text-white'>
        {/* Balance */}
            <div className='flex w-4/5 justify-around items-center mt-7'>
                <div onClick={() => App.setShowErc(App.showErc ? false : true)} className='flex cursor-pointer justify-center items-center border-2 border-blue-900 border-opacity-60 p-3 bg-black bg-opacity-70 rounded-lg'>
                  ...
                  <h1 className='ml-2 mr-2 text-lg font-medium'>{App.currency}</h1> 
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-chevron-down" viewBox="0 0 16 16">
                  <path fill-rule="evenodd" d="M1.646 4.646a.5.5 0 0 1 .708 0L8 10.293l5.646-5.647a.5.5 0 0 1 .708.708l-6 6a.5.5 0 0 1-.708 0l-6-6a.5.5 0 0 1 0-.708z"/>
                  </svg>
                </div>
                {/* Balance section */}
                <div  className='flex border-2 border-blue-900 border-opacity-60 p-3 bg-black rounded-lg bg-opacity-70 justify-center items-center'>
        ...
        <div className={`${App.showErc ? '' : "hidden"} flex w-4/5 justify-between items-center mt-5`}>
            <input onChange={(e) => App.setErcTokenAddress(e.target.value)} value={App.ercTokenAddress} className='w-3/4 p-3 bg-black border-2 border-blue-900 border-opacity-60 bg-opacity-70 outline-none rounded-lg' placeholder='Please Enter ERC20 Token Address'/>
            {App.ercLoading ?
                ...
                :
                (App.tokenChanged ? 
                <div onClick={App.removeToken} className='flex cursor-pointer justify-around items-center w-1/4 p-2 ml-4 bg-red-600 bg-opacity-70 border-blue-900 border-opacity-60 text-xl font-medium rounded-lg'>
                    Remove
                </div>
                :
                <div onClick={App.selectToken} className='flex cursor-pointer justify-around items-center w-1/4 p-2 ml-4 bg-blue-600 bg-opacity-70 border-blue-900 border-opacity-60 text-xl font-medium rounded-lg'>
                    Select
                </div>
                )
            }
        ...
        {/* Transfer Button */}
        { App.txLoading ?
            <div className='flex mt-4 w-4/5 cursor-pointer justify-center items-center p-2 bg-green-700 hover:bg-green-900 bg-opacity-70 border-2 border-blue-900 border-opacity-80 text-xl font-medium rounded-lg'>
                <TailSpin 
                    width={28}
                    height={28}
                    color={'white'}
        ...
        :
            <div onClick={App.transferAmount} className='flex mt-4 w-4/5 cursor-pointer justify-center items-center p-2 bg-green-700 hover:bg-green-900 bg-opacity-70 border-2 border-blue-900 border-opacity-80 text-xl font-medium rounded-lg'>
                Transfer
            </div>
        }

        {/*  RecentTx section /save ERCtx data section */}
        <div className={ `${App.showRecentTx ? '' : 'hidden'}  bg-black rounded-lg bg-opacity-60 border-2 border-blue-900 border-opacity-80 w-4/5 mt-2 ` }>
            ...
            {App.saveTxLoad ?
                <div className='flex justify-center bg-green-700  hover:bg-green-900 font-medium font-mono bg-opacity-80 h-full w-1/6 py-1 mr-2 rounded-md'>
                    <TailSpin
                        height={18}
                        width={18}
                        color={'white'}
                ...
                :
                <button onClick={App.savetx} className='bg-green-700  hover:bg-green-900 font-medium font-mono bg-opacity-80 h-full w-1/6 py-1 mr-2 rounded-md'>Save</button>
            }
                <button onClick={() => App.setShowRecentTx(false)} className='bg-red-700  hover:bg-red-900 font-medium font-mono bg-opacity-80 h-full w-1/6 py-1 mr-2 rounded-md'>Ignore</button>
            </div>
        ...
        {/* Error & Message */}
        <p className='text-red-600 max-w-full flex overflow-hidden p-2 text-sm mt-2 px-3'>{App.error}</p>
